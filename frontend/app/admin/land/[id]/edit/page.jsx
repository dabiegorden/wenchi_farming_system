...
export default function EditLand({ params }) {
  ...
  useEffect(() => {
    const fetchLandDetails = async () => {
      try {
        ...
        // Set original image URL if it exists
        if (land.imageUrl) {
          setOriginalImageUrl(land.imageUrl)
          setImagePreview(`http://localhost:5000${land.imageUrl}`)
        }
        ...
        setFormData({
          ...
          coordinates: land.coordinates || { latitude: "", longitude: "" },
          size: land.size ? (typeof land.size === "object" ? land.size.value : land.size) : "",
          unit: land.size && land.size.unit ? land.size.unit : "hectare",
          soilType: land.soilType || "",
          soilPh: land.soilPh || "",
          soilMoisture: land.soilMoisture ? land.soilMoisture.value : "",
          currentCrop: land.currentCrop ? land.currentCrop._id : "",
          plantingDate: land.plantingDate ? new Date(land.plantingDate).toISOString().split("T")[0] : "",
          expectedHarvestDate: land.expectedHarvestDate
            ? new Date(land.expectedHarvestDate).toISOString().split("T")[0]
            : "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        // Keep the original image if no new one was selected
        formDataToSubmit.append("imageUrl", originalImageUrl)
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to update land plot")
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center space-x-2">
        <Button variant="outline" size="icon" asChild>
          <Link href={`/admin/land/${landId}`}>
            <ArrowLeft className="h-4 w-4" />
          </Link>
        </Button>
        <h1 className="text-3xl font-bold">Edit Land Plot</h1>
      </div>
        ...
        <Card className="mt-6">
          ...
          <CardFooter className="flex justify-end space-x-2 border-t px-6 py-4">
            <Button variant="outline" asChild>
              <Link href={`/admin/land/${landId}`}>Cancel</Link>
            </Button>
